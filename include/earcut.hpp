...
using TransformedMesh = std::pair<Vector2, Mesh>;

TransformedMesh earcut_single(const Way& w) {
  using namespace std;
...
template <typename Pred>
std::vector<TransformedMesh> earcut_collection(WayFilterView<Pred>&& buildings) {
  std::vector<TransformedMesh> meshes;

  for (const Way& w : buildings) {
    meshes.push_back(earcut_single(w));
  }
